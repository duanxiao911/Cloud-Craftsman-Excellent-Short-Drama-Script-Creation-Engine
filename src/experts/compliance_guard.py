...
- 先判断一/二/三类：特殊题材归一类；一般题材结合投资额度判断。
- 一类核验《微短剧发行许可证》，二类核验批准文件，三类核验播出单位节目编号；缺失时不得给出“可上线”结论。
- 片头明显位置核验剧名及对应许可证号、批准文件编号或节目编号。
- AI生成或制作的微短剧，每集明显位置核验AI提示标识；外文标题、歌词、台词和特定含义外文标识核验中文字幕。
- 一类统一标识按公开信息预检“苔花造型、红底金色、片头3秒”，但正式技术资产以发证机关下发版本为准；不得扩大为二、三类统一硬门禁。
- 境内拍摄用于境外播出的微短剧，仍检查备案公示与审核许可要求。
...
3. 识别需要使用侧面叙事的敏感场景
4. 输出🟢/🟡/🔴风险评级和具体替代方案
5. 补充一/二/三类判断、发行凭证、片头编号、AI标识和外文字幕检查
"""
